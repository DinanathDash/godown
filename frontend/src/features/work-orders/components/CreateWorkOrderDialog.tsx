...
import { z } from 'zod';
import { useCreateWorkOrder, useLocations, useItems, useOperationsUsers } from '../api/work-orders';
...
export function CreateWorkOrderDialog() {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      window.alert(e.response?.data?.error?.message || 'Failed to create work order');
    }
  };

  // Mocking fallback for users if endpoint doesn't exist. Assuming 1 default operations user from seed.
  const displayUsers = usersData || [{ id: 'mock-uuid', name: 'Ops User (Mock)' }];
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              placeholder="e.g. 50"
            />
            {errors.requiredQty && <p className="text-xs text-red-500">{errors.requiredQty.message}</p>}
          </div>
