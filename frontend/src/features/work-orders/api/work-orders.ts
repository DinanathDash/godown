...
export function useWorkOrders(params?: { page?: number; limit?: number; status?: WorkOrderStatus }) {
  return useQuery({
    queryKey: ['work-orders', params],
    queryFn: async () => {
      const res = await apiClient.get<{ data: WorkOrder[]; meta: { page: number; limit: number; total: number; totalPages: number } }>('/work-orders', { params });
      return res.data;
    },
  });
}
...
export function useCreateWorkOrder() {
  ...
  return useMutation({
    mutationFn: async (data: { locationId: string; itemId: string; requiredQty: number; assignedToId: string }) => {
...
export function useOperationsUsers() {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      const res = await apiClient.get<{ data: { id: string; name: string; email: string; role: string }[] }>('/users');
